...
- 签名要求：请求和接收数据均需要签名认证
###2.2.	检测地址
>http://api.yunclean.cn:8080/audit
###2.3.	请求描述
####	Http的Post调用参数说明
####键值对参数
| 参数名 | 参数类型 | 说明 |
...
###4.2.	接口地址
>http://api.yunclean.cn:8080/audit-result
###4.3.	请求描述
####Http的Post调用参数说明
####键值对参数
|参数名称	|参数类型	|说明|
...
##5.	代码示例
###5.1.	Java代码示例
**示例附件**：yunclean-api-java-2.0.zip
下载地址：
>https://dn-yunclean.qbox.me/documents/docs_guide/yuncleanapi-java-2.0.rar

###5.2.	PHP代码示例
**示例附件**： yunclean-api-php-2.0.zip
下载地址：
>https://dn-yunclean.qbox.me/documents/docs_guide/yunclean-api-php-2.0.zip
##6.	对接常见问题
用户在调用“数据接入接口”时可能出现一些问题，主要是一些参数的格式以及签名算法不正确，总结如下。

>1）	version：本版本固定为2.0
2）	timestamp：不是时间戳格式，需要传入标准的时间格式“yyyy-MM-dd HH:mm:ss”。
3）	contents：是一个json集合格式。即使只有1条数据，也是以[]包含具体的数据，如：
```
"contents": [
            {
                "type": "text",
 ...
 ```
4）sign：详见1.2节说明（当返回4005时，代表签名计算有误）。
